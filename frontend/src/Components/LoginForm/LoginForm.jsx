...
const LoginForm = () => {
  ...
  const handleSubmitForm = async (values) => {
    try {
      const response = await fetch('http://ec2-174-129-118-14.compute-1.amazonaws.com/v1/auth/authenticate', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const getStudent = await fetch(
          `http://ec2-174-129-118-14.compute-1.amazonaws.com/v1/categories/1/customers/${userTypeId}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
