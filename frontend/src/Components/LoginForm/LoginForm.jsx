...
const LoginForm = () => {
  ...
  const handleSubmitForm = async (values) => {
    try {
      const response = await fetch('ec2-107-21-139-55.compute-1.amazonaws.com/v1/auth/authenticate', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const getStudent = await fetch(
          `ec2-107-21-139-55.compute-1.amazonaws.com/v1/categories/1/customers/${userTypeId}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
